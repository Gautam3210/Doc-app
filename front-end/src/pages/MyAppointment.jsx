
import React, { useContext } from 'react';
import { AppContext } from '../context/AppContext';

const MyAppointment = () => {
  const { doctors } = useContext(AppContext);

  return (
    <div>
      <p className='pb-3 mt-12 font-medium text-zinc-700 border-b'>My Appointment</p>
      <div>
        {doctors && doctors.length > 0 ? (
          doctors.slice(0, 3).map((item, index) => (
            <div className='grid grid-cols-[1fr_2fr] sm:flex sm:gap-6 py-2 border-b' key={index}>
              <div>
                <img
                  className='w-32 bg-indigo-50'
                  src={item?.image || 'https://via.placeholder.com/100'}
                  alt={item?.name || 'Doctor'}
                />
              </div>
              <div className='flex-1 text-sm text-zinc-600'>
                <p className='text-neutral-800 font-semibold'>{item?.name || 'No Name'}</p>
                <p>{item?.speciality || 'No Speciality'}</p>
                <p className='text-zinc-700 font-medium mt-1'>Address:</p>
                <p className='text-xs'>{item?.address?.line1 || 'No Address Line 1'}</p>
                <p className='text-xs'>{item?.address?.line2 || 'No Address Line 2'}</p>
                <p className='text-xs mt-1'>
                  <span className='text-sm text-neutral-700 font-medium'>Date & Time:</span> 25 July 2025 | 8:30 PM
                </p>
              </div>
              <div></div>

              <div className='flex flex-col gap-2 justify-end'>
                <button className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border hover:bg-primary hover:text-white transition-all duration-300'>Pay Online</button>
                <button className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border hover:bg-red-600 hover:text-white transition-all duration-300'>Cancle Appointment </button>
              </div>
            </div>   
          ))
        ) : (
          <p>No doctors available.</p>
        )}
      </div>
    </div>
  );
}

export default MyAppointment;
